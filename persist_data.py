...
def resolve_db_path(*, env_key: str = "DB_PATH", default_filename: str = "data.db") -> str:
    """DB yo'li: env > Railway volume > /data."""
...
def migrate_legacy_db(target_path: str, *extra_legacy: str) -> str | None:
    """Bo'sh/yangi DB bo'lsa — eski fayllardan nusxa (bir martalik)."""
    ...
    candidates: list[str] = []
    for p in (*extra_legacy, base, "data.db", os.path.join("/app", base), os.path.join("/app", "data.db")):
...
def startup_sqlite_backup(db_path: str, backup_dir: str | None = None) -> str | None:
    """Har ishga tushishda DB nusxasi (/data/backups)."""
...
def bootstrap_persistence(
    db_path: str,
    *,
    legacy_names: tuple[str, ...] = ("data.db",),
) -> dict:
    """Ishga tushishda: papka, migratsiya, zaxira, volume ogohlantirish."""
