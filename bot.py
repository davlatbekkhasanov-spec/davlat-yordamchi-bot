...
async def save_number(message: Message):
    ...
    # session: keyin yakunlashda guruhga ketadi
    ...
    await message.answer(
        ...
        f"Период (2-санада): <b>{period_sum}</b>\n"
        f"{plan_txt}\n\n"
...
async def finalize_report(message: Message):
    ...
    # eng kuchli kategoriya (session bo'yicha)
    best_cat, best_add = max(agg.items(), key=lambda x: x[1])
    total_added = sum(agg.values())

    # umumiy kechaga taqqos (faqat ishlatilgan kategoriyalar bo'yicha)
    today_total = 0
    yday_total = 0
    for cat in agg.keys():
    ...
    # tartib: CATEGORIES bo'yicha (chiroyli)
    ...
    lines.append("")
    lines.append(f"✅ Сессия жами: {total_added}")
...
# ============================================================
# АДМИН: /report /top /leaders /stats
# ============================================================
...
async def stats_cmd(message: Message):
    ...
    lines.append("")
    lines.append("💡 Мотивация: ҳамма бир хил шароитда эмас. Мақсад — барқарор ўсиш 🤝")
